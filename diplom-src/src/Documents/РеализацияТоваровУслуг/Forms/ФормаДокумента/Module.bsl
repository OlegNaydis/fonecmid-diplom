...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//++олег   
	//если в документе есть только услуги - откроем сркзу на второй странице
...
Процедура ВКМ_ЗаполнитьНаСервере()
	...
	Если ВидДоговора<>Перечисления.ВидыДоговоровКонтрагентов.ВКМ_АбонентскоеОбслуживание Тогда
	     ОбщегоНазначения.СообщитьПользователю("Заполнение возможно только для договоров абонентского обслуживания");
		 возврат;
	иначе	
	     ТекДок = РеквизитФормыВЗначение("Объект");   
		 ТекДок.ВКМ_ВыполнитьАвтозаполнение();  
